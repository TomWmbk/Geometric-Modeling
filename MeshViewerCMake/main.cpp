...
void menu(int item)
{
	...
	case MENU_INFLATE:
		...
		refresh(); break;
	case MENU_SIMPLIFY:
		m->simplify((int)m->faces.size() / 2); refresh(); break;
	case MENU_OPENFILE:
...
void display() 
{
	...
	if (drawsilhouette)
	{
		...
		if (silhouette_dirty || camera_moved)
		{
			...
			silhouette_edge_count = (unsigned int)silhouette_edges.size();

			if (silhouette_buffer == 0) glGenBuffers(1, &silhouette_buffer);
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, silhouette_buffer);
			if (silhouette_edge_count > 0)
				glBufferData(GL_ELEMENT_ARRAY_BUFFER, silhouette_edge_count * sizeof(GLuint),
					&silhouette_edges[0], GL_DYNAMIC_DRAW);
		}

		if (silhouette_edge_count > 0 && buffers[BUFFER_VERTICES])
		{
			glLineWidth(4.0);
			...
			glDrawElements(GL_LINES, silhouette_edge_count, GL_UNSIGNED_INT, 0);
		}
	}
